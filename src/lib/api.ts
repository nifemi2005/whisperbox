...
import type { RegisterPayload, LoginPayLoad, Session, User } from '../types/auth'
import type { MessageResponse, ConversationSummary, EncryptedPayload } from '../types/message'
...
export async function searchUsers(query: string): Promise<User[]> {
  return request<User[]>(`${ENDPOINTS.SEARCH_USERS}?q=${encodeURIComponent(query)}`)
}
...
export async function getMessageHistory(userId: string): Promise<MessageResponse[]> {
